...
def _maybe_send_to_broker(db: Session, *, order_id: int, symbol: str, side: str, qty: float, order_type: str | None):
    # 지금 단계에서는 "received -> sending -> (sent/failed)" 전이만 책임지고,
    # okx 실제 전송은 main.py 안에 존재하는 okx_place_order()가 있을 때만 호출한다.
    if _is_dry_run():
        return
...
"""
    ...
    # Inject broker call inside /tv after _create_order_if_new, before "if not created:"
    if_pat = r"\n(\s+if not created:)"
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
"""
    # indent to match the surrounding block (we'll detect indentation from the if not created line)
