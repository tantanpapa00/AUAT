...
### 3. 실적 및 밸류에이션 분석
- **최근 실적 추이**: 제공된 3개년 매출/영업이익/순이익 분석
- **수익성 지표**: ROE, 영업이익률 해석
- **밸류에이션**: PER/PBR 현재 수준, 과거 평균 대비, 동종업계 대비
- **증권사 컨센서스**: 웹검색 결과 기재 (없으면 "증권사 컨센서스 데이터 없음")
...
- 종합 신호: 매수/매도/중립 중 하나
- 3가지 시나리오 (가격대 명시):
  1. 상승 시나리오: 저항선 {resistance1} 돌파 시 목표가
  2. 횡보 시나리오: {support1}~{resistance1} 박스권
  3. 하락 시나리오: 지지선 {support1} 이탈 시 하방 목표
...
2. **증권사 목표가는 웹검색 결과만**. 찾지 못하면 "데이터 없음" 표시.
3. 제공된 재무 지표 수치를 정확히 인용하세요.
4. 기술적 분석은 전체 분량의 30% 이하로 축약하세요.
5. 핵심 요약은 반드시 위 6개 항목(기업정체/실적/밸류에이션/성장동력/리스크/기술신호)으로 구성.
6. 마크다운 형식 사용 (# ## ### 헤딩, 테이블, 글머리 기호).
7. **절대 금지**: "웹검색을 통해", "보고서를 작성하겠습니다" 등 과정 설명.
8. 바로 [SECTION_1_4] 구분자로 시작하세요.
"""
    return prompt
