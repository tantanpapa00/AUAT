...
### 2. 기업 분석 (사업 이해)
- **사업 개요**: 기업 개요 데이터를 바탕으로 주요 사업, 제품/서비스 설명
- **시장 포지션**: 업종 내 위치, 경쟁력 (추측 금지, 데이터 기반만)
- **매출 구성**: 제품별/사업부별 매출 비중 (데이터 있는 경우만)

### 3. 실적 및 밸류에이션 분석
- **연간 실적 추이**: 제공된 3개년 매출/영업이익/순이익 분석
  | 항목 | 전전년 | 전년 | 당해/전망 |
  |------|--------|------|-----------|
  | 매출액 | X | X | X |
  | 영업이익 | X | X | X |
...
- **수익성 지표**: ROE, 영업이익률 해석
- **밸류에이션**: PER/PBR 현재 수준, 과거 평균 대비, 동종업계 대비
- **증권사 컨센서스** (웹검색 결과만 사용):
  | 증권사명 | 투자의견 | 목표가 | 날짜 |
  |----------|----------|--------|------|
  ⚠️ 필수: 증권사명 없이 목표가만 쓰지 마세요. 부정적 의견도 포함하세요.
...
- 3가지 시나리오 (52주 고저 기반으로 현실적 조정폭 제시):
  ...
  ⚠️ 주의: 급등주의 경우 30% 이상 조정 시나리오 반드시 포함

### 6. 면책조항
"본 보고서는 투자 참고 자료로만 활용하시기 바라며, 특정 종목의 매수/매도를 권유하지 않습니다. AI가 생성한 내용으로 오류가 있을 수 있습니다."

---

## 작성 규칙 (필수)

1. **사업 내용은 제공된 기업 개요만 사용**. 추측/지어내기 절대 금지.
2. **증권사 목표가는 [증권사명 + 목표가 + 투자의견] 세트로 기재**. 출처 불명 데이터 금지.
   - 부정적 의견도 반드시 포함 (균형 잡힌 시각)
   - 찾지 못하면 "증권사 컨센서스 데이터 없음" 표시
3. **실적 테이블**: 매출액/영업이익/순이익/EPS/영업이익률/ROE 필수 포함.
4. **시나리오 조정폭**: 52주 고저 기반으로 현실적 가격대 제시. 급등주는 30% 이상 조정 시나리오 필수.
5. **RS(상대강도)**: 기술적 분석에 시장 대비 강세/약세 판단 포함.
6. 기술적 분석은 전체 분량의 30% 이하로 축약하세요.
7. 핵심 요약은 반드시 6개 항목(기업정체/실적/밸류에이션/성장동력/리스크/기술신호)으로 구성.
8. **면책조항은 리포트 맨 마지막에 1번만** (중복 금지).
9. 마크다운 형식 사용 (# ## ### 헤딩, 테이블, 글머리 기호).
10. **절대 금지**: "웹검색을 통해", "보고서를 작성하겠습니다" 등 과정 설명.
