...
# Admin Router (명령서61 - 모듈 분리)
from .routers.admin import router as admin_router
app.include_router(admin_router)

# Market KR Router (명령서61 - 모듈 분리)
from .routers.market_kr import router as market_kr_router
app.include_router(market_kr_router)

# Market US Router (명령서61 - 모듈 분리)
from .routers.market_us import router as market_us_router
app.include_router(market_us_router)

# Market Misc Router (ETF, Crypto) (명령서61 - 모듈 분리)
from .routers.market_misc import router as market_misc_router
app.include_router(market_misc_router)

# Screener Router (명령서61 - 모듈 분리)
...
# /api/market/kr/overview → routers/market_kr.py로 이동
# /api/market/us/* → routers/market_us.py로 이동
# /api/market/etf, /api/market/crypto → routers/market_misc.py로 이동


# /api/screener/* → routers/screener.py로 이동
...
**기업 사업 내용 관련:**
- 아래 "기업 개요"에 제공된 정보만 사용하세요.
- 제공되지 않은 사업 내용, 제품, 고객사, 경쟁사를 추측하거나 지어내지 마세요.
- 잘 모르는 내용은 "기업 개요 데이터 참조" 또는 생략하세요.
- 예: 한미반도체는 "반도체 제조사"가 아니라 "반도체 후공정 장비 회사"입니다. 기업 개요를 정확히 읽으세요.
...
1. 모든 재무 데이터는 반드시 "연결 재무제표(consolidated)" 기준으로 작성하라.
   ...
   - 예시: 한미반도체 2024년 연결 매출 5,589억 vs 별도 매출 559억 → 반드시 5,589억을 사용.

2. 숫자 현실성 검증을 반드시 수행하라:
   - 영업이익률이 60%를 초과하면 → 별도 재무제표이거나 계산 오류일 가능성이 극히 높다. 재확인하라.
...
3. 웹검색에서 재무 데이터를 가져올 때 (서버 데이터가 없는 경우):
   ...
   - 교차 검증 결과 숫자가 크게 다르면(2배 이상), 더 큰 숫자가 연결 기준일 가능성이 높다.
...
### [기업 분석 심층 규칙]
1. 시장 포지션 분석 시 반드시 다음을 포함하라:
   - 글로벌 시장 점유율 수치 (예: "TC 본더 시장 점유율 약 71.2%")
   - 출처가 있으면 명시 (예: "테크인사이츠에 따르면")
   - 경쟁사 대비 기술적 우위 설명 (예: "120여 개 특허", "자체 부품 생산")

2. 핵심 제품/기술 설명 시:
   - 현재 주력 제품뿐 아니라 차세대 개발 중인 제품도 언급하라
   - 제품별 매출 기여도가 확인되면 포함하라
   - ASP(평균판매단가) 변화 추이도 포함하라

3. 고객 다변화 전략:
   - 현재 고객별 매출 비중 + 향후 변화 방향을 구체적 수치로 기술하라
   - 신규 고객사 확보 현황 (예: "마이크론 최우수 협력사 수상")
   - 해외 매출 비중 변화 추이

4. 수익성 구조:
   - 영업이익률이 업계 평균 대비 어느 수준인지 비교하라
   - 높은 수익성의 원인을 분석하라 (독점적 지위, 기술 장벽 등)
   - 부채비율, 유보율 등 재무 건전성 지표도 언급하라

### [증권사 컨센서스 강화 규칙]
1. 증권사명은 반드시 실명으로 기재하라.
   - "기타 강세 리포트", "기타 분석기관", "일부 증권사" 같은 익명/일반명칭 사용 금지.
2. 최소 4개 이상의 실명 증권사/기관 의견을 포함하라.
   - 국내: LS증권, 유진투자증권, 리딩투자증권, KB증권, 하나증권, 신한투자증권 등
   - 해외: BofA 메릴린치, JP모건, 모건스탠리, UBS 등
3. 각 증권사별로 반드시 포함할 정보:
   - 증권사명 (실명)
   - 투자의견 (매수/중립/매도)
   - 목표주가 (구체적 금액)
   - 핵심 근거 (1~2줄)
4. 목표주가 범위와 현재가 괴리율을 분석하라.
   - 예: "증권사 목표주가 범위: 15만~30만 원, 현재가 대비 -8%~+54%"
5. 실명을 확인할 수 없는 증권사 의견은 포함하지 마라.

### [증권사 실명 보완 규칙]
1. "실명을 확인할 수 없는 증권사 의견은 포함하지 마라"의 올바른 해석:
   - ✅ 올바른 해석: 익명으로 적지 말고, 웹검색을 더 해서 실명을 찾아라.
   - ❌ 잘못된 해석: 실명을 모르면 아예 빼버려라. (이렇게 하면 증권사가 1개만 남음)

2. 증권사 정보를 찾는 웹검색 전략:
   - 검색어: "{name} 목표주가", "{name} 증권사 리포트", "{name} 투자의견"
   - 검색어: "{name} target price analyst"
   - 네이버 금융 > 종목 > 투자의견 탭 활용
   - 최소 3회 이상 웹검색하여 증권사 실명을 확보하라.

3. 테이블에는 반드시 실명 증권사만 기재하되, 최소 3개 이상이어야 한다.
   - 3개 미만이면 웹검색을 추가로 수행하라.
   - 그래도 3개를 못 찾으면, 찾은 만큼만 기재하되 "기타", "일부" 같은 익명 행은 넣지 마라.

4. 증권사 테이블 아래에 요약 문장 추가:
   - "증권사 목표주가 범위: X만~Y만 원 (현재가 대비 -Z%~+W%)"
   - 이 문장은 반드시 테이블 숫자와 일치해야 한다.

### [본문-테이블 일관성 절대 규칙]
1. 연간 실적 테이블의 숫자와 본문의 숫자는 100% 동일해야 한다.
   - 테이블에 "2025A 매출 5,767억"이면, 본문에도 반드시 5,767억으로 쓰라.
   - 절대로 테이블과 다른 숫자를 본문에 쓰지 마라.
2. 숫자 작성 순서: 테이블을 먼저 완성하고, 본문에서 테이블의 숫자를 인용하라.
3. 컨센서스 매출액과 실적 매출액이 다를 수 있다. 이 경우:
   - 테이블에는 확정 실적 또는 가장 신뢰도 높은 수치를 기재
   - 컨센서스 수치는 "증권가 컨센서스 기준 매출 X억 원"으로 별도 언급
   - 둘을 혼동하지 마라.

### [빈칸 최소화 규칙]
1. 연간 실적 테이블 24칸(6항목 × 4개년) 중 빈칸("-")이 8개를 넘으면 안 된다.
   - 순이익: 웹검색에서 확인되면 반드시 기재. 영업이익에서 추정해도 된다.
   - EPS: 순이익 ÷ 발행주식수로 계산 가능하면 기재하라.
   - ROE: 순이익 ÷ 자기자본으로 계산 가능하면 기재하라.
2. 분기별 실적은 최소 4분기 이상 숫자를 기재하라.
3. 2023A(과거 확정 실적)는 반드시 채워라. 과거 실적은 변하지 않는다.

---

22. 바로 [SECTION_1_4] 구분자로 시작하세요.
"""
