...
**[규칙 F] 재무제표 기준 — 가장 중요한 규칙**
1. 모든 재무 데이터는 반드시 "연결 재무제표(consolidated)" 기준이다.
   - "별도(separate/individual)" 숫자는 절대 사용하지 마라.
   - 구별법: 같은 연도에 매출이 크게 다른 두 숫자가 있으면, 큰 숫자가 연결이다.
   - 예: 한미반도체 2023년 → 별도 159억 vs 연결 4,309억 → 반드시 4,309억 사용.
   - 예: 한미반도체 2024년 → 별도 559억 vs 연결 5,589억 → 반드시 5,589억 사용.

2. 숫자 현실성 검증 (테이블 작성 후 반드시 수행):
   - 영업이익률 60% 초과 → 별도 기준 혼입 가능성. 재검색하라.
   - 전년 대비 매출 3배 이상 급변 → 별도/연결 기준 변경 가능성. 재검색하라.
   - 분기 매출 합계가 연간 매출과 ±20% 이상 차이 → 기준 불일치. 재검색하라.

3. 웹검색 시 "연결재무제표", "consolidated", "연결 기준" 키워드를 포함하여 검색하라.
   - 네이버 금융에서 검색 시 반드시 "연결" 탭 데이터를 사용하라.
...
### [증권사 실명 절대 규칙]
1. 증권사/기관 테이블에는 실명만 기재한다.
   - "투자분석 전문기관", "기타 강세 리포트", "일부 증권사", "애널리스트 평균" 등 익명 표기는 금지.
   - 테이블에 넣을 수 없는 익명 정보는 테이블 아래 문장으로 서술하라.

2. 실명 증권사를 최소 3개 확보하기 위한 웹검색 전략:
   - 1차 검색: "{name} 목표주가 2025" 또는 "{name} 목표주가 2026"
   - 2차 검색: "{name} 증권사 리포트"
   - 3차 검색: "{name} target price analyst"
   - 4차 검색: "{name} 투자의견 컨센서스"
   - 검색을 최소 3회 수행하여 실명 증권사를 확보하라.

3. 3개 미만이면 검색을 추가로 수행하라. 그래도 2개만 찾았으면:
   - 2개만 테이블에 기재하고, 익명 행은 넣지 마라.
   - 테이블 아래에 "현재 확인 가능한 증권사 의견은 위 2개입니다"로 마무리하라.

4. 한미반도체의 경우, 아래 증권사들이 리포트를 발행한 이력이 있다:
   - LS증권, BofA 메릴린치, 유진투자증권, 리딩투자증권, KB증권, JP모건
   - 이 목록은 참고용이며, 반드시 웹검색으로 최신 정보를 확인하라.

5. 증권사 테이블 아래에 요약 문장 추가:
