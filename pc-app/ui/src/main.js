...
document.getElementById('btn-ai-analysis')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!reqResult.success) {
            const isUS = getAppMode() === 'US';
            throw new Error(reqResult.error || (isUS ? 'Request failed' : '요청 실패'));
        }
        ...
        <div class="ai-report" style="font-size:14px;line-height:1.7;color:var(--text-primary);">
            ...
            <div style="display:flex;justify-content:flex-end;margin-bottom:16px;gap:8px;">
                <button onclick="window.downloadAiReportPdf('${jobId}', '${stockName}', '${stockCode}')"
                    ...
                    <span>📄</span> ${isUS ? 'PDF Download' : 'PDF 다운로드'}
                </button>
...
window.shareAiReport = async function(stockName, stockCode) {
    ...
    const shareText = isUS
        ? `📊 ${stockName}(${stockCode}) AI Analysis\n\nAI investment analysis report generated by BBooster.`
        : `📊 ${stockName}(${stockCode}) AI 종합분석\n\nBBooster에서 생성된 AI 투자 분석 리포트입니다.`;

    if (navigator.share) {
        try {
            await navigator.share({
                title: isUS ? `${stockName} AI Analysis` : `${stockName} AI 분석`,
                text: shareText
            });
        } catch (e) {
        ...
        try {
            ...
            showToast(isUS ? 'Analysis copied to clipboard' : '분석 내용이 클립보드에 복사되었습니다', 'success');
        } catch (e) {
            showToast(isUS ? 'Share failed' : '공유 실패', 'error');
        }
    }
};
...
async function sendAiQuestion(text) {
    ...
    responseArea.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;color:var(--text-muted);">
            ...
            <span id="ai-chat-progress">🤖 ${isUS ? 'AI is thinking...' : 'AI가 생각 중...'}</span>
        </div>
    `;
...
async function downloadAiReportPdf(jobId, stockName, stockCode) {
    ...
    try {
        const savePath = await invoke('download_ai_pdf', {
            jobId: jobId,
            fileName: `${stockName || 'BBooster'}_${stockCode || jobId}_${isUS ? 'Analysis' : '분석리포트'}.pdf`
        });
        showToast(isUS ? 'PDF saved: ' + savePath : 'PDF 저장 완료: ' + savePath, 'success');
    } catch (e) {
        console.error('[PDF Download]', e);
        showToast(isUS ? 'PDF download failed: ' + e : 'PDF 다운로드 실패: ' + e, 'error');
    }
...
async function shareAiReport(stockName, stockCode) {
    ...
    if (!answerEl) {
        showToast(isUS ? 'No content to share' : '공유할 내용이 없습니다', 'error');
        return;
    }
    ...
    // 공유 텍스트 구성
    const shareText = isUS
        ? `📊 ${stockName} (${stockCode}) AI Analysis Report\n\n${textContent}\n\n─────────────────\n🤖 BBooster AI Analysis\n📱 https://qube-system.com`
        : `📊 ${stockName} (${stockCode}) AI 분석 리포트\n\n${textContent}\n\n─────────────────\n🤖 BBooster AI 분석\n📱 https://qube-system.com`;

    try {
        // 클립보드에 복사
        await navigator.clipboard.writeText(shareText);
        showToast(isUS ? 'Copied to clipboard!' : '클립보드에 복사되었습니다!', 'success');
    } catch (e) {
        ...
        try {
            ...
            showToast(isUS ? 'Copy failed: ' + err : '복사 실패: ' + err, 'error');
        }
...
function updateScreenerFiltersUI(market) {
    ...
    if (chipGroups) {
        ...
        Object.entries(filterDefs).forEach(([key, def]) => {
            ...
            if (!categories[cat]) categories[cat] = [];
            categories[cat].push({ key, label: def.label, labelEn: def.labelEn });
        });
            ...
            if (filters && filters.length > 0) {
                html += `
                    <div class="filter-chip-group">
                        ...
                        <div class="chip-row">
                            ${filters.map(f => `<button class="filter-chip" data-filter="${f.key}">${f.labelEn && isUS ? f.labelEn : f.label}</button>`).join('')}
                        </div>
                    </div>
                `;
            }
