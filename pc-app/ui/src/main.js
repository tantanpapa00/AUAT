...
document.getElementById('btn-trend-run-backtest')?.addEventListener('click', async () => {
    ...
    try {
        ...
        const result = await invoke('run_trend_backtest', {
            ...
            cash_use_pct: cfg.cash_use_pct,
            // 슈퍼트렌드
            st_atr_len: cfg.st_atr_len,
            st_factor: cfg.st_factor,
            // 피라미딩
            use_pyramiding: cfg.use_pyramiding,
            max_pyr_entries: cfg.max_pyr_entries,
            pyr_high_len: cfg.pyr_high_len,
            pyr_cooldown: cfg.pyr_cooldown,
            pyr_refill_after_sell: cfg.pyr_refill_after_sell,
            pyr_weights: cfg.pyr_weights,
            // 추세전환 전량매도
            use_st_exit: cfg.use_st_exit,
            // 부분익절 (TP1)
            use_tp1: cfg.use_tp1,
            tp1_pct: cfg.tp1_pct,
            tp1_sell_pct: cfg.tp1_sell_pct,
            // 과매수구간 분할매도
            use_spo_split: cfg.use_spo_split,
            sell_tranches: cfg.sell_tranches,
            max_sell_tranches: cfg.max_sell_tranches,
            after_max_sell: cfg.after_max_sell,
            use_profit_gate: cfg.use_profit_gate,
            // 손절
            stop_type: cfg.stop_type,
            hard_sl_pct: cfg.hard_sl_pct,
            atr_stop_len: cfg.atr_stop_len,
