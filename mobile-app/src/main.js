// =====================================================
// BBooster Mobile App (Capacitor)
// Tauri invoke 제거 → HTTP API 직접 호출
// =====================================================
...
console.log('[Chart] Chart.js + ChartDataLabels 등록 완료 (Mobile)');

// =====================================================
// 모바일 환경 감지
// =====================================================
const IS_MOBILE = true; // Capacitor 앱
const IS_CAPACITOR = typeof window !== 'undefined' && window.Capacitor !== undefined;

// =====================================================
// 네이티브 커맨드 대체 구현 (Tauri → Capacitor/웹)
// =====================================================
const NATIVE_COMMAND_HANDLERS = {
    // 외부 링크 열기 → Capacitor Browser
    'open_url': async (args) => {
        await Browser.open({ url: args.url });
        return { success: true };
    },
    ...
    // 서버 상태 → API 호출로 대체
    'check_server_health': async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/health`, { timeout: 5000 });
            return { ok: res.ok, message: res.ok ? '서버 연결됨' : '서버 응답 오류', latency_ms: 0 };
        } catch (e) {
            return { ok: false, message: '연결 실패', latency_ms: 0 };
        }
    },
    'get_server_status': async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/diag/home`);
            if (res.ok) {
                const data = await res.json();
                return { running: true, estop: data.estop || false, dry_run: data.dry_run || false };
            }
            return { running: false, estop: false, dry_run: false };
        } catch (e) {
            return { running: false, estop: false, dry_run: false };
        }
    },
    ...
    // E-STOP → API 호출
    'set_estop': async (args) => {
        const res = await fetch(`${API_BASE_URL}/api/system/estop?value=${args.enabled ? 1 : 0}`, { method: 'POST' });
        return args.enabled;
    },

    // 모바일에서 지원 안 함 (무시)
    'start_server': async () => ({ success: true, message: '모바일에서는 서버 시작 불필요' }),
    'stop_server': async () => ({ success: true, message: '모바일에서는 서버 중지 불필요' }),
    'open_logs_folder': async () => ({ success: false, message: '모바일에서 지원 안 함' }),
    'export_diagnostic': async () => ({ success: false, message: '모바일에서 지원 안 함' }),
    'download_ai_pdf': async () => ({ success: false, message: '모바일에서 지원 안 함' }),

    // 로컬 계정 키 → localStorage 사용 (보안 주의: 프로덕션에서는 Capacitor SecureStorage 권장)
    'save_api_key': async (args) => {
        localStorage.setItem(`api_key_${args.exchange}`, JSON.stringify(args));
        return { success: true };
    },
    'get_api_key': async (args) => {
        const data = localStorage.getItem(`api_key_${args.exchange}`);
        return data ? JSON.parse(data) : null;
    },
    'save_account_keys': async (args) => {
        localStorage.setItem(`account_keys_${args.accountId}`, JSON.stringify(args));
        return { success: true };
    },
    'get_account_keys': async (args) => {
        const data = localStorage.getItem(`account_keys_${args.accountId}`);
        return data ? JSON.parse(data) : null;
    },
    'delete_account_keys': async (args) => {
        localStorage.removeItem(`account_keys_${args.accountId}`);
        return { success: true };
    },
    'list_local_accounts': async () => {
        // localStorage에서 account_keys_ 접두사로 시작하는 항목 찾기
        const accounts = [];
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key && key.startsWith('account_keys_')) {
                try {
...
// 네이티브 전용 커맨드 Set (대체 구현 키 목록)
const NATIVE_COMMANDS = new Set(Object.keys(NATIVE_COMMAND_HANDLERS));

// =====================================================
// 외부 링크 열기 헬퍼 (Tauri open → Capacitor Browser)
// =====================================================
...
const API_COMMAND_MAP = {
    ...
    'get_user_info': (args) => api.getUserInfo(args.accessToken),
    'refresh_auth_token': (args) => api.refreshAuthToken(args.refreshToken),
    'verify_password': (args) => api.verifyPassword(args.accessToken, args.password),

    // Portfolio
    'get_portfolio_summary': (args) => api.getPortfolioSummary(args.accessToken),
    'get_portfolio_profit_rate': (args) => api.getPortfolioProfitRate(args.accessToken, args.period),
    'get_portfolio_chart': (args) => api.getPortfolioChart(args.accessToken, args.period),
    'get_holdings': (args) => api.getHoldings(args.accessToken),
    'get_trade_history': (args) => api.getTradeHistory(args.accessToken, args.limit, args.offset),
    'get_asset_trades': (args) => api.getAssetTrades(args.accessToken, args.assetId, args.limit),
    'get_portfolio_history': (args) => api.getPortfolioHistory(args.accessToken, args.days),

    // Accounts
    'get_accounts_list': (args) => api.getAccountsList(args.accessToken),
    'register_api_key': (args) => api.registerApiKey(args.accessToken, args.exchange, args.apiKey, args.apiSecret, args.passphrase, args.accountNumber),
    'delete_api_key': (args) => api.deleteApiKey(args.accessToken, args.accountId),
    'test_account_connection': (args) => api.testAccountConnection(args.accessToken, args.accountId),

    // Strategies
    'get_active_strategies': (args) => api.getActiveStrategies(args.accessToken),
    'get_strategies': (args) => api.getStrategies(args.accessToken),
    'save_strategy': (args) => api.saveStrategy(args.accessToken, args.strategy),
    'toggle_strategy': (args) => api.toggleStrategy(args.accessToken, args.strategyId),
    'delete_strategy': (args) => api.deleteStrategy(args.accessToken, args.strategyId),
    'toggle_asset': (args) => api.toggleAsset(args.accessToken, args.assetId),
    'delete_asset': (args) => api.deleteAsset(args.accessToken, args.assetId),
    'create_strategy_with_params': (args) => api.createStrategyWithParams(args.accessToken, args),
    'save_signal_params': (args) => api.saveSignalParams(args.accessToken, args.assetId, args.params),
    'create_asset': (args) => api.createAsset(args.accessToken, args),

    // Emergency
    'emergency_stop': (args) => api.emergencyStop(args.accessToken),

    // Webhook
    'get_webhook_logs': (args) => api.getWebhookLogs(args.accessToken, args.limit),
    'get_webhook_url': (args) => api.getWebhookUrl(args.accessToken),
    ...
    'get_market_events': (args) => api.getMarketEvents(args.accessToken, args.days),
    'get_market_timeline': (args) => api.getMarketTimeline(args.accessToken),
    'get_market_signal': (args) => api.getMarketSignal(args.accessToken),
    'get_market_big_picture': (args) => api.getMarketBigPicture(args.accessToken),
    'get_market_signal_history': (args) => api.getMarketSignalHistory(args.accessToken, args.days),
    'get_market_breadth_data': (args) => api.getMarketBreadthData(args.accessToken, args.period),
    'get_market_breadth_with_index': (args) => api.getMarketBreadthWithIndex(args.accessToken, args.period),
    'get_market_trend_maintain': (args) => api.getMarketTrendMaintain(args.accessToken),
    'get_market_sector_analysis': (args) => api.getMarketSectorAnalysis(args.accessToken),
    'get_market_rs_ranking': (args) => api.getMarketRsRanking(args.accessToken, args.market, args.limit),
    'get_market_sector_stocks': (args) => api.getMarketSectorStocks(args.accessToken, args.sector, args.limit),
    'get_market_investors_data': (args) => api.getMarketInvestorsData(args.accessToken, args.period),
    'get_market_trading_value_data': (args) => api.getMarketTradingValueData(args.accessToken, args.period, args.type),
    'get_market_etf': (args) => api.getMarketEtf(args.accessToken, args.sort, args.limit),
    'get_market_crypto': (args) => api.getMarketCrypto(args.accessToken, args.sort, args.limit),
    'get_analysis_rs': (args) => api.getAnalysisRs(args.accessToken, args.market, args.limit),
    'get_analysis_new_high': (args) => api.getAnalysisNewHigh(args.accessToken),
    'get_analysis_valuation': (args) => api.getAnalysisValuation(args.accessToken, args.market, args.sort, args.limit),

    // Screener
    'get_screener': (args) => api.getScreener(args.accessToken, args.filters || args),
    ...
    'get_stock_financials_kr': (args) => api.getStockFinancialsKr(args.code, args.finType),
    ...
    'get_stock_statement_kr': (args) => api.getStockStatementKr(args.code, args.periodType),
    ...
    // AI
    'get_ai_usage': (args) => api.getAiUsage(args.accessToken),
    'request_ai_analysis': (args) => api.requestAiAnalysis(args.accessToken, args),
    'check_ai_status': (args) => api.checkAiStatus(args.accessToken, args.jobId),
    'request_ai_chat': (args) => api.requestAiChat(args.accessToken, args),
    'get_ai_recommendations': (args) => api.getAiRecommendations(args.accessToken, args.market),

    // Watchlist
    'get_watchlist_groups': (args) => api.getWatchlistGroups(args.accessToken),
    'create_watchlist_group': (args) => api.createWatchlistGroup(args.accessToken, args.name, args.description),
    'delete_watchlist_group': (args) => api.deleteWatchlistGroup(args.accessToken, args.groupId),
    'get_watchlist_items': (args) => api.getWatchlistItems(args.accessToken, args.groupId),
    'add_watchlist_item': (args) => api.addWatchlistItem(args.accessToken, args.groupId, args.exchange, args.symbol),
    'remove_watchlist_item': (args) => api.removeWatchlistItem(args.accessToken, args.groupId, args.itemId),

    // Backtest
    'run_backtest': (args) => api.runBacktest(args.accessToken, args),
    'run_mr_backtest': (args) => api.runMrBacktest(args.accessToken, args),
    'run_trend_backtest': (args) => api.runTrendBacktest(args.accessToken, args),
    'run_custom_backtest': (args) => api.runCustomBacktest(args.accessToken, args),
    'preload_candles': (args) => api.preloadCandles(args.accessToken, args.exchange, args.symbol, args.timeframe, args.days),

    // Premium Strategy Engine
    'get_premium_configs': (args) => api.getPremiumConfigs(args.accessToken),
    'get_premium_config': (args) => api.getPremiumConfig(args.accessToken, args.assetId),
    'create_premium_config': (args) => api.createPremiumConfig(args.accessToken, args),
    'update_premium_config': (args) => api.updatePremiumConfig(args.accessToken, args.assetId, args),
    'delete_premium_config': (args) => api.deletePremiumConfig(args.accessToken, args.assetId),
    'get_strategy_state': (args) => api.getStrategyState(args.accessToken, args.assetId),
    'reset_strategy_state': (args) => api.resetStrategyState(args.accessToken, args.assetId),
    'get_scheduler_status': (args) => api.getSchedulerStatus(args.accessToken),
    'start_scheduler': (args) => api.startScheduler(args.accessToken),
    'stop_scheduler_premium': (args) => api.stopSchedulerPremium(args.accessToken),
    'register_to_scheduler': (args) => api.registerToScheduler(args.accessToken, args.assetId),
    'trigger_signal': (args) => api.triggerSignal(args.accessToken, args.assetId),
    'get_signal_events': (args) => api.getSignalEvents(args.accessToken, args.assetId, args.limit),

    // Admin
    'admin_get_users': (args) => api.adminGetUsers(args.accessToken, args.page, args.limit, args.search),
    'admin_update_user_plan': (args) => api.adminUpdateUserPlan(args.accessToken, args.userId, args.plan),
    'admin_get_system_status': (args) => api.adminGetSystemStatus(args.accessToken),
    'admin_get_stats': (args) => api.adminGetStats(args.accessToken),
    'admin_get_recent_users': (args) => api.adminGetRecentUsers(args.accessToken),

    // Exchange Rate
    'get_exchange_rate': () => api.getExchangeRate(),

    // KIS Order Settings
    'save_kis_order_settings': (args) => api.saveKisOrderSettings(args.accessToken, args),
    'get_kis_order_settings': (args) => api.getKisOrderSettings(args.accessToken, args.accountId),

    // Notifications (명령서63)
    'get_notifications': (args) => api.getNotifications(args.accessToken, args.limit, args.offset, args.unreadOnly),
    'mark_notification_read': (args) => api.markNotificationRead(args.accessToken, args.notificationId),
    'mark_all_notifications_read': (args) => api.markAllNotificationsRead(args.accessToken),
    'delete_notification': (args) => api.deleteNotification(args.accessToken, args.notificationId),
    'delete_all_notifications': (args) => api.deleteAllNotifications(args.accessToken),
};

// invoke 래퍼 - API / 네이티브 분기 (모바일용)
async function invoke(cmd, args = {}) {
    // 네이티브 커맨드 → 모바일 대체 구현
    if (NATIVE_COMMANDS.has(cmd)) {
        const handler = NATIVE_COMMAND_HANDLERS[cmd];
        if (handler) {
            try {
                return await handler(args);
            } catch (err) {
                console.error(`[Native] ${cmd} 오류:`, err);
                throw err;
            }
        }
    }
    ...
    // 매핑 없는 경우 → 오류
    console.error(`[invoke] 매핑되지 않은 커맨드: ${cmd}`);
    throw new Error(`Unknown command: ${cmd}. 모바일 앱에서는 Tauri invoke를 사용할 수 없습니다.`);
}
...
async function loadUserWebhookUrl() {
    ...
    try {
        if (auth.accessToken) {
            ...
            webhookUrlEl.textContent = `${API_BASE_URL}/api/webhook/${auth.user.id}`;
        }
    } catch (error) {
...
document.getElementById('btn-open-terms')?.addEventListener('click', () => open(`${API_BASE_URL}/terms`));
document.getElementById('btn-open-privacy')?.addEventListener('click', () => open(`${API_BASE_URL}/privacy`));
...
async function loadMarketKr() {
    ...
    try {
        ...
        contentEl.innerHTML = `
            ...
            <!-- 오늘의 특징주 탭 -->
            <div class="se-tab-content" id="tab-featured-stocks">
            ...
            <div class="se-tab-content" id="tab-sector" style="display:none;">
                <div class="se-sector-controls">
                    <div class="se-sector-sort">
                        <label>정렬:</label>
                        <select id="sector-sort-by">
                            <option value="change">등락률</option>
                            <option value="volume">거래대금</option>
                        </select>
                    </div>
                    <div class="se-sector-filter">
                        <button class="se-filter-btn active" data-filter="all">전체</button>
                        <button class="se-filter-btn" data-filter="up">상승</button>
                        <button class="se-filter-btn" data-filter="down">하락</button>
                    </div>
                </div>
        ...
        document.querySelectorAll('.se-subtab').forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                if (tabId === 'trend-maintain') {
                    ...
                    loadSectorDataUnified();
                } else if (tabId === 'featured-stocks') {
                    const activeType = document.querySelector('.se-featured-tab.active')?.dataset.type || 'gainers';
                    const market = document.getElementById('featured-market-filter')?.value || 'ALL';
                    loadFeaturedStocksKr(activeType, market);
                }
            });
        });
        ...
        // 오늘의 특징주 로드 (기본 탭)
        loadFeaturedStocksKr('gainers', 'ALL');

        // 오늘의 특징주 탭 이벤트
        setupFeaturedStocksEvents();

        // 오늘의 업종 탭 이벤트
        setupSectorTabEvents();
...
// 특징주 로드 (상승률/하락률/거래량)
async function loadFeaturedStocksKr(type = 'gainers', market = 'ALL') {
    ...
    try {
        ...
        console.log('[loadFeaturedStocks]', type, market, data);
                    ...
                    ${displayStocks.map((s, idx) => {
                        ...
                        return `
                            <tr class="stock-row" data-code="${s.code}" data-name="${s.name}">
                                <td class="rank-col">${s.rank || idx + 1}</td>
                                <td class="name-col">
                                    <span class="stock-name">${s.name}</span>
                                    <span class="stock-code">${s.code}</span>
                                    ${s.market ? `<span class="stock-market ${s.market.toLowerCase()}">${s.market}</span>` : ''}
                                </td>
...
// 특징주 탭 이벤트 설정
function setupFeaturedStocksEvents() {
    // 탭 버튼 이벤트
    document.querySelectorAll('.se-featured-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            document.querySelectorAll('.se-featured-tab').forEach(t => t.classList.remove('active'));
            tab.classList.add('active');
            const type = tab.dataset.type;
            const market = document.getElementById('featured-market-filter')?.value || 'ALL';
            loadFeaturedStocksKr(type, market);
        });
    });

    // 시장 필터 이벤트
    const marketFilter = document.getElementById('featured-market-filter');
    if (marketFilter) {
        marketFilter.addEventListener('change', () => {
            const activeType = document.querySelector('.se-featured-tab.active')?.dataset.type || 'gainers';
            loadFeaturedStocksKr(activeType, marketFilter.value);
        });
    }
}

// ============================================================================
// 오늘의 업종 (Step 5) - 통합 리스트
// ============================================================================

// 업종 통합 로드
async function loadSectorDataUnified() {
    ...
    try {
        const data = await invokeWithTimeout('get_market_sectors', { accessToken: auth.accessToken || '' }, 10000);
        console.log('[loadSectorDataUnified] data:', data);
        ...
        // 기본 정렬: 등락률 내림차순
        renderSectorList(sectors, 'change', 'all');
    } catch (error) {
        console.error('[loadSectorDataUnified] error:', error);
        listEl.innerHTML = `<div class="error-state">${error?.message || error}</div>`;
    }
}

// 섹터 리스트 렌더링 (정렬/필터 적용)
function renderSectorList(sectors, sortBy = 'change', filter = 'all') {
    const listEl = document.getElementById('kr-sector-unified');
    if (!listEl) return;
    ...
    if (filter === 'up') {
        filtered = sectors.filter(s => (s.change_percent || 0) > 0);
    } else if (filter === 'down') {
        filtered = sectors.filter(s => (s.change_percent || 0) < 0);
    }
        ...
        <table class="se-sector-table">
            ...
            <tbody>
                ${filtered.map(s => {
                    const changeClass = (s.change_percent || 0) >= 0 ? 'profit' : 'loss';
                    const sign = (s.change_percent || 0) >= 0 ? '+' : '';
                    const volStr = formatSectorTradingValue(s.trading_value || 0);

                    return `
                        <tr class="sector-row clickable" data-sector="${s.name}">
                            <td class="name-col">${s.name}</td>
                            <td class="change-col ${changeClass}">${sign}${(s.change_percent || 0).toFixed(2)}%</td>
                            <td class="volume-col">${volStr}</td>
                        </tr>
                    `;
                }).join('')}
            </tbody>
    ...
    // 섹터 클릭 이벤트 (섹터 종목 조회)
    listEl.querySelectorAll('.sector-row').forEach(row => {
        row.addEventListener('click', async () => {
            const sectorName = row.dataset.sector;
            // 섹터 상세 모달 또는 확장
            showSectorStocksModal(sectorName);
        });
    });
}
...
async function showSectorStocksModal(sectorName) {
    ...
    try {
        ...
        if (resp?.success) {
            const stocks = [...(resp.top_gainers || []), ...(resp.top_losers || [])].slice(0, 10);
            if (stocks.length > 0) {
                const stockList = stocks.map((s, i) => {
                    const sign = (s.change_percent || 0) >= 0 ? '+' : '';
                    return `${i+1}. ${s.name}: ${sign}${(s.change_percent || 0).toFixed(1)}%`;
                }).join('\n');
        ...
        console.error('[showSectorStocksModal]', err);
    }
}

// 업종 탭 이벤트 설정
function setupSectorTabEvents() {
    // 정렬 드롭다운
    const sortSelect = document.getElementById('sector-sort-by');
    if (sortSelect) {
        sortSelect.addEventListener('change', () => {
            const filter = document.querySelector('.se-filter-btn.active')?.dataset.filter || 'all';
            renderSectorList(sectorDataCache, sortSelect.value, filter);
        });
    }
    ...
    document.querySelectorAll('.se-filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const sortBy = document.getElementById('sector-sort-by')?.value || 'change';
            renderSectorList(sectorDataCache, sortBy, btn.dataset.filter);
        });
    });
}

// 종목 상세 페이지 이동
function goToStockDetail(code, name, market = 'KR') {
    // 종목 상세 탭 활성화 + 데이터 로드
    document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
    const stockBtn = document.querySelector('.nav-btn[data-page="stock"]');
    if (stockBtn) stockBtn.classList.add('active');

    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
    const stockPage = document.getElementById('page-stock');
    if (stockPage) stockPage.classList.add('active');

    // 국내/해외 탭 설정
    const krTab = document.querySelector('.stock-tab[data-tab="kr"]');
    const usTab = document.querySelector('.stock-tab[data-tab="us"]');
    if (market === 'KR' && krTab) {
        krTab.click();
    } else if (market === 'US' && usTab) {
        usTab.click();
    }
...
async function loadCompanyInfoKr(code) {
    ...
    try {
        ...
        if (response && response.data) {
            ...
            // 1. 상단 요약 카드 (StockEasy 스타일)
            const priceColorClass = (data.change_pct || 0) >= 0 ? 'positive' : 'negative';
            const summaryHtml = `
                <div class="company-header-card">
                    <div class="company-main-row">
                        <div class="company-main-item">
                            <div class="company-label">시가총액</div>
                            <div class="company-value-lg">${data.market_cap || '-'}</div>
                        </div>
                        <div class="company-main-item">
                            <div class="company-label">현재가</div>
                            <div class="company-value-lg ${priceColorClass}">
                                ${data.price?.toLocaleString() || '-'}원
                                <span class="company-change-pct">(${(data.change_pct || 0) >= 0 ? '+' : ''}${(data.change_pct || 0).toFixed(2)}%)</span>
                            </div>
                        </div>
                    </div>
                    <div class="company-sub-row">
                        <span>↗ 52주 고가 <strong class="text-red">${data.high_52w?.toLocaleString() || '-'}원</strong></span>
                        <span>↘ 52주 저가 <strong class="text-blue">${data.low_52w?.toLocaleString() || '-'}원</strong></span>
                        <span>🌐 외인 <strong>${data.foreign_ratio > 0 ? data.foreign_ratio.toFixed(2) + '%' : '-'}</strong></span>
                    </div>
                </div>
            `;
            ...
            const overviewHtml = `
                <div class="company-overview-card">
                    <div class="overview-header-row">
                        <h3 class="overview-title">🏢 기업 개요</h3>
                        ${data.description_title ? `<span class="overview-subtitle">${data.description_title}</span>` : ''}
                    </div>
...
// 해외 종목 기업 정보 로드 (StockEasy 스타일 + 한글 번역)
async function loadCompanyInfoUs(ticker) {
    ...
    try {
        ...
        if (response && response.data) {
            ...
            // 1. 상단 요약 카드 (StockEasy 스타일 - 국내와 동일 구조)
            const summaryHtml = `
                <div class="company-header-card">
                    <div class="company-main-row">
                        <div class="company-main-item">
...
// =====================================================
// Mobile Tab Bar Navigation (Capacitor)
// =====================================================

// 모바일 페이지 전환
window.switchMobilePage = function(pageName) {
    // 탭 활성화 상태 변경
    ...
    const pageMapping = {
        ...
        'strategy': 'page-strategy-setup',
        'market': 'page-market-analysis',
        'ai': 'page-ai-assistant'
    };

    // 사이드바 메뉴 클릭 시뮬레이션
    const sidebarItem = document.querySelector(`.sidebar-nav a[href="#${pageMapping[pageName]}"]`);
    if (sidebarItem) {
        sidebarItem.click();
    } else {
        // 직접 페이지 전환
        showPage(pageMapping[pageName] || 'page-home');
    }

    console.log('[Mobile] Page switched to:', pageName);
};
...
function initMobileTabBar() {
    ...
    if (tabBar) {
        tabBar.style.display = isMobile ? 'block' : 'none';
    }
    ...
    window.addEventListener('resize', () => {
        ...
        if (tabBar) {
            tabBar.style.display = nowMobile ? 'block' : 'none';
        }
    });
}

// 페이지 전환 시 탭 상태 동기화
function syncMobileTabWithPage(pageId) {
    const reverseMapping = {
        'page-home': 'home',
        'page-screener': 'screener',
        'page-strategy-setup': 'strategy',
        'page-market-analysis': 'market',
        'page-ai-assistant': 'ai'
    };

    const tabName = reverseMapping[pageId];
    if (tabName) {
        document.querySelectorAll('.mobile-tab-bar .tab-item').forEach(tab => {
    ...
    try {
        ...
        if (permission.receive === 'granted') {
            ...
            PushNotifications.addListener('registration', async (token) => {
                ...
                // 서버에 토큰 전송 (로그인 상태일 때만)
                if (auth.isLoggedIn() && auth.accessToken) {
                    try {
                        await api.apiPost('/api/push/register', {
                            token: token.value,
            ...
            PushNotifications.addListener('pushNotificationReceived', (notification) => {
                ...
                // 인앱 알림 표시
                showMobileToast(notification.title || notification.body || '새 알림');
            });
            ...
            PushNotifications.addListener('pushNotificationActionPerformed', (action) => {
                ...
                if (data) {
                    // 알림 타입에 따라 페이지 이동
                    if (data.type === 'trade' || data.type === 'signal') {
                        switchMobilePage('strategy');
                    } else if (data.type === 'market') {
                        switchMobilePage('market');
...
// 로그인 성공 후 푸시 초기화 (기존 로그인 함수에서 호출)
window.initPushAfterLogin = initPushNotifications;

console.log('[Push] Push notification module loaded');
