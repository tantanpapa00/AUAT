# patch_replace_poll_wrapper_v4.py
# Usage:
#   cd C:\autobot
#   python .\patch_replace_poll_wrapper_v4.py

from __future__ import annotations
import sys
from pathlib import Path
from datetime import datetime

MAIN = Path(r"C:\autobot\app\main.py")

def die(msg: str) -> None:
    print(f"[patch-wrapper-v4] ERROR: {msg}", file=sys.stderr)
    sys.exit(1)

def backup(path: Path) -> Path:
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bak = path.with_suffix(path.suffix + f".bak.{ts}")
    bak.write_bytes(path.read_bytes())
    return bak

def main():
    if not MAIN.exists():
        die(f"not found: {MAIN}")

    txt = MAIN.read_text(encoding="utf-8-sig")

    start = txt.find("# [POLL_CHANGES_WRAPPER_V1]")
    if start < 0:
        die("marker not found: # [POLL_CHANGES_WRAPPER_V1]")

    end = txt.find("# [ORDER_POLL_WORKER_V1]", start)
    if end < 0:
        end2 = txt.find("\n@app.", start)
        end = end2 if end2 >= 0 else len(txt)

    head = txt[:start]
    tail = txt[end:]

    new_block = r'''
# [POLL_CHANGES_WRAPPER_V1]
def poll_orders_once(*, limit: int = 20) -> dict:
    """
    poll_orders_once 래퍼 (v4):
    - sent/partial + okx_order_id 있는 주문만 대상으로 합니다.
    - 대상이 0건이면 OKX 호출 없이 즉시 반환합니다 (changes 모드 타임아웃 방지).
    - 대상이 있으면 기존 구현(_poll_orders_once_impl)을 1회 실행한 뒤,
      전/후 스냅샷 비교로 "변경분"만 반환합니다.
    """
    candidate_ids: list[int] = []
    before: dict[int, dict] = {}

    # 1) BEFORE snapshot (candidates)
    try:
        db_gen = get_db()
        db = next(db_gen)
        try:
            rows = db.execute(text("""
                select id, asset_id, symbol, market, side, qty, order_type,
                       status, okx_order_id, okx_state, filled_qty, avg_px, last_checked_at
                  from orders
                 where okx_order_id is not null
                   and status in ('sent','partial')
                 order by coalesce(last_checked_at, to_timestamp(0)) asc, id asc
                 limit :lim
            """), {"lim": limit}).mappings().all()

            for r in rows:
                oid = int(r["id"])
                candidate_ids.append(oid)
                before[oid] = {
                    "status": r.get("status"),
                    "okx_state": r.get("okx_state"),
                    "filled_qty": r.get("filled_qty"),
                    "avg_px": r.get("avg_px"),
                    "last_checked_at": r.get("last_checked_at"),
                }
        finally:
            try:
                db_gen.close()
            except Exception:
                pass
    except Exception:
        # snapshot 실패 시에도 안전하게 즉시 반환(변경분 계산 불가)
        return {"ok": True, "count": 0, "items": [], "scanned": 0, "note": "changes_only | snapshot_failed"}

    # ✅ 핵심: 후보 0건이면 OKX 호출 없이 즉시 반환
    if not candidate_ids:
        return {"ok": True, "count": 0, "items": [], "scanned": 0, "note": "changes_only | no_candidates"}

    # 2) Run existing implementation (may update DB)
    try:
        _poll_orders_once_impl(limit=limit)
    except Exception as e:
        # impl이 죽어도 poll-now는 500으로 죽지 않게
        return {"ok": False, "count": 0, "items": [], "scanned": len(candidate_ids), "note": f"impl_failed: {e}"}

    # 3) AFTER snapshot + diff
    changes: list[dict] = []
    try:
        db_gen2 = get_db()
        db2 = next(db_gen2)
        try:
            # IN (:id0,:id1,...) 안전 구성 (any(:ids) 사용 안함)
            params = {}
            ph = []
            for i, oid in enumerate(candidate_ids):
                k = f"id{i}"
                params[k] = int(oid)
                ph.append(f":{k}")

            sql = f"""
                select id, asset_id, symbol, market, side, qty, order_type,
                       status, okx_order_id, okx_state, filled_qty, avg_px, last_checked_at, reason
                  from orders
                 where id in ({", ".join(ph)})
                 order by id asc
            """
            after_rows = db2.execute(text(sql), params).mappings().all()

            for r in after_rows:
                oid = int(r["id"])
                b = before.get(oid, {})
                a_status = r.get("status")
                a_state  = r.get("okx_state")
                a_fq     = r.get("filled_qty")
                a_apx    = r.get("avg_px")

                changed = (
                    a_status != b.get("status")
                    or (a_state or None) != (b.get("okx_state") or None)
                    or (a_fq or None) != (b.get("filled_qty") or None)
                    or (a_apx or None) != (b.get("avg_px") or None)
                )

                if changed:
                    changes.append({
                        "order_id": oid,
                        "asset_id": r.get("asset_id"),
                        "symbol": r.get("symbol"),
                        "market": r.get("market"),
                        "side": r.get("side"),
                        "qty": r.get("qty"),
                        "order_type": r.get("order_type"),
                        "from_status": b.get("status"),
                        "to_status": a_status,
                        "from_okx_state": b.get("okx_state"),
                        "to_okx_state": a_state,
                        "filled_qty": a_fq,
                        "avg_px": a_apx,
                        "okx_order_id": r.get("okx_order_id"),
                        "last_checked_at": r.get("last_checked_at"),
                        "reason": r.get("reason"),
                    })
        finally:
            try:
                db_gen2.close()
            except Exception:
                pass
    except Exception:
        # diff 실패해도 안전하게 반환
        return {"ok": True, "count": 0, "items": [], "scanned": len(candidate_ids), "note": "changes_only | diff_failed"}

    return {
        "ok": True,
        "count": len(changes),
        "items": changes,
        "scanned": len(candidate_ids),
        "note": "changes_only",
    }
'''.lstrip("\n")

    bak = backup(MAIN)
    MAIN.write_text(head + new_block + tail, encoding="utf-8")
    print("[patch-wrapper-v4] OK")
    print(f"[patch-wrapper-v4] main.py -> {MAIN} (backup: {bak.name})")

if __name__ == "__main__":
    main()
