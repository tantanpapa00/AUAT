...
# --- find def _set_order_status ---
m = re.search(r"^def\s+_set_order_status\s*\(", src, flags=re.M)
if not m:
    raise SystemExit("ERR: cannot find def _set_order_status(")

start = m.start()

# --- find end of _set_order_status: next top-level def/class after it ---
m_next = re.search(r"^\s*(def|class)\s+\w+", src[m.end():], flags=re.M)
if not m_next:
    raise SystemExit("ERR: cannot find next def/class after _set_order_status")

insert_at = m.end() + m_next.start()
...
# --- inject call right after _create_order_if_new(...) assignment in tv_webhook ---
...
tmp.replace(path)
print("OK: injected _maybe_send_to_broker (after _set_order_status) + call after _create_order_if_new")
