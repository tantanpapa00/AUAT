...
# ============================================================
# [HOTPATCH_W3D1] Enable OKX broker + ensure_orders_table
# - Fix: send_impl_missing (no okx_place_order in globals)
# - Fix: /api/orders 500 due to missing _ensure_orders_table or missing cols
# - Safe: create/alter IF NOT EXISTS only (non-destructive)
# ============================================================

def _hotpatch_bool(v: str, default: bool=False) -> bool:
    try:
        s = (v or "").strip().lower()
...
if "okx_place_order" not in globals():
    ...
    def _okx_env():
        # supports both OKX_API_* and legacy OKX_* names
        def _clean(v: str) -> str:
            v2 = (v or "").strip()
            if len(v2) >= 2 and ((v2[0] == v2[-1]) and v2[0] in ("\"", "\'")):
                v2 = v2[1:-1].strip()
            return v2
        base = _clean(os.getenv("OKX_BASE_URL") or "https://www.okx.com").rstrip("/")
        key  = _clean(os.getenv("OKX_API_KEY") or os.getenv("OKX_KEY") or "")
        sec  = _clean(os.getenv("OKX_API_SECRET") or os.getenv("OKX_SECRET") or "")
        pas  = _clean(os.getenv("OKX_API_PASSPHRASE") or os.getenv("OKX_PASSPHRASE") or "")
        sim  = _clean(os.getenv("OKX_SIMULATED") or "0")
        timeout = float(_clean(os.getenv("OKX_TIMEOUT") or "10") or "10")
        # treat whitespace/quoted blanks as missing
        if not key or not sec or not pas:
            raise RuntimeError("okx_env_missing: need OKX_API_KEY/OKX_API_SECRET/OKX_API_PASSPHRASE (or OKX_KEY/OKX_SECRET/OKX_PASSPHRASE)")
    ...
    def _okx_sign(secret: str, prehash: str) -> str:
        mac = _hmac.new(secret.encode("utf-8"), prehash.encode("utf-8"), digestmod="sha256").digest()
        return _base64.b64encode(mac).decode("utf-8")
    ...
    def _dec_str(x) -> str:
        """Return a non-scientific string for numbers."""
        try:
    ...
    def okx_place_order(*, symbol: str, side: str, qty: float, order_type: str = "market", payload: dict | None = None) -> dict:
        """Place OKX spot order. Returns OKX JSON (code=0)."""
        ...
        if isinstance(payload, dict):
            clid = payload.get("clOrdId") or payload.get("clordid") or payload.get("client_oid") or payload.get("clientOrderId")
            if clid:
                body["clOrdId"] = str(clid)

        if s_side == "buy" and ord_type == "market":
            body["tgtCcy"] = "base_ccy"
        ...
        if ord_type == "limit":
            ...
            if isinstance(payload, dict):
                px = payload.get("price") or payload.get("px")
            if px is None:
                raise RuntimeError("limit_requires_price")
            body["px"] = _dec_str(px)

        body_json = _json.dumps(body, separators=(",", ":"), ensure_ascii=False)
        ts = _okx_ts()
        prehash = f"{ts}POST{path}{body_json}"
        sign = _okx_sign(sec, prehash)
        headers = _okx_headers(ts, sign, key=key, pas=pas, sim=sim)

        resp = _requests.post(url, headers=headers, data=body_json.encode("utf-8"), timeout=timeout)
        if resp.status_code != 200:
            raise RuntimeError(f"okx_http_error status={resp.status_code} body={resp.text}")
        data = resp.json()
        if str(data.get("code")) != "0":
            raise RuntimeError(f"okx_error code={data.get('code')} msg={data.get('msg')} data={data.get('data')}")
        return data
    ...
    def okx_get_order(*, symbol: str, okx_order_id: str | None = None, okx_clord_id: str | None = None) -> dict:
        """Fetch OKX order by ordId or clOrdId."""
        if _requests is None:
            raise RuntimeError("requests_missing")
        base, key, sec, pas, sim, timeout = _okx_env()
        path = "/api/v5/trade/order"
...
# ============================================================
# [HOTPATCH_W3D1_OKX_PREFLIGHT] block submit on OKX env mismatch (50101)
# - cache preflight result (TTL seconds)
# - wrap okx_place_order to enforce preflight before sending
# - reason code: OKX_ENV_MISMATCH / OKX_AUTH_ERROR
# ============================================================

if "_okx_preflight_cache" not in globals():
    _okx_preflight_cache = {"ts": 0.0, "ok": None, "msg": ""}

def okx_preflight(*, ttl_sec: int = 30) -> dict:
    """Check OKX environment/auth quickly. Returns {ok:bool, msg:str} with TTL cache."""
    import time
    now = time.time()
    try:
        if _okx_preflight_cache["ok"] is not None and (now - float(_okx_preflight_cache["ts"])) < ttl_sec:
            return {"ok": bool(_okx_preflight_cache["ok"]), "msg": str(_okx_preflight_cache["msg"])}

        # minimal call that triggers 50101 if environment mismatch
        base, key, sec, pas, sim, timeout = _okx_env()
        ...
        _okx_preflight_cache.update({"ts": now, "ok": True, "msg": "ok"})
        return {"ok": True, "msg": "ok"}

    except Exception as e:
        msg = str(e)
        _okx_preflight_cache.update({"ts": now, "ok": False, "msg": msg})
        return {"ok": False, "msg": msg}

# wrap okx_place_order (enforce preflight)
if "okx_place_order" in globals() and "_okx_place_order_wrapped" not in globals():
    _okx_place_order_wrapped = True
    _okx_place_order_original = okx_place_order

    def okx_place_order(*, symbol: str, side: str, qty: float, order_type: str = "market", payload: dict | None = None) -> dict:
        chk = okx_preflight(ttl_sec=int(os.getenv("OKX_PREFLIGHT_TTL", "30") or "30"))
        if not chk.get("ok", False):
            # normalize reason for 50101
            msg = str(chk.get("msg", ""))
            if "50101" in msg or "environment" in msg.lower():
                raise RuntimeError("OKX_ENV_MISMATCH: " + msg)
            raise RuntimeError("OKX_AUTH_ERROR: " + msg)
        return _okx_place_order_original(symbol=symbol, side=side, qty=qty, order_type=order_type, payload=payload)

# optional: expose diag endpoint
try:
    if "app" in globals():
        @app.get("/api/diag/okx-preflight")
        def diag_okx_preflight():
            return {"ok": True, "check": okx_preflight(ttl_sec=0)}
except Exception:
    pass


# ============================================================
# [HOTPATCH_W3D2_BAL_GUARD] Block OKX submit when balance insufficient
# - Prevent OKX 51008 spam by pre-checking balances
# - Standardize reason prefix: INSUFFICIENT_BAL / PRICE_UNAVAILABLE
# - Uses OKX /api/v5/account/balance and /api/v5/market/ticker (TTL caches)
# ============================================================

if "_okx_bal_cache" not in globals():
    _okx_bal_cache = {"ts": 0.0, "data": None, "msg": ""}

if "_okx_px_cache" not in globals():
    _okx_px_cache = {}  # instId -> {"ts": float, "px": float}

def _hotpatch_float(v: str, default: float) -> float:
    try:
        return float((v or "").strip())
    except Exception:
        return default

def okx_get_balances(*, ttl_sec: int = 10) -> dict:
    """Return OKX balance payload with TTL cache."""
    import time
    now = time.time()
    try:
        if _okx_bal_cache["data"] is not None and (now - float(_okx_bal_cache["ts"])) < ttl_sec:
            return {"ok": True, "data": _okx_bal_cache["data"], "msg": "cached"}
        ...
        j = resp.json()
        if str(j.get("code")) != "0":
            raise RuntimeError(f"okx_error code={j.get('code')} msg={j.get('msg')} data={j.get('data')}")

        _okx_bal_cache.update({"ts": now, "data": j, "msg": "ok"})
        return {"ok": True, "data": j, "msg": "ok"}
    except Exception as e:
        _okx_bal_cache.update({"ts": now, "data": None, "msg": str(e)})
        return {"ok": False, "data": None, "msg": str(e)}

def okx_avail_ccy(ccy: str) -> float:
    """Extract available balance(=availBal) for currency."""
    res = okx_get_balances(ttl_sec=int(os.getenv("OKX_BAL_TTL", "10") or "10"))
    if not res.get("ok"):
        return 0.0
    j = res.get("data") or {}
    # OKX format: data[0].details[*].ccy, .availBal
    try:
        data = (j.get("data") or [])
        if not data:
            return 0.0
        ...
        ccy_u = (ccy or "").upper()
        for d in details:
            if (d.get("ccy") or "").upper() == ccy_u:
                return float(d.get("availBal") or 0.0)
    except Exception:
        pass
    return 0.0

def okx_last_price(instId: str, *, ttl_sec: int = 10) -> float | None:
    """Fetch last price using /api/v5/market/ticker (TTL cache per instId)."""
    import time
    now = time.time()
    k = (instId or "").strip()
    if not k:
        return None
    try:
        hit = _okx_px_cache.get(k)
        if hit and (now - float(hit.get("ts", 0.0))) < ttl_sec:
            return float(hit.get("px"))

        base = os.getenv("OKX_BASE_URL") or "https://www.okx.com"
        url = f"{base}/api/v5/market/ticker?instId={k}"
        resp = _requests.get(url, timeout=float(os.getenv("OKX_TIMEOUT", "10") or "10"))
        if resp.status_code != 200:
            return None
        j = resp.json()
        if str(j.get("code")) != "0":
        ...
        if not data:
            return None
        px = float(data[0].get("last") or 0.0)
        if px <= 0:
            return None
        _okx_px_cache[k] = {"ts": now, "px": px}
        return px
    except Exception:
        return None

def _inst_base_quote(instId: str) -> tuple[str, str]:
    # OKX spot instId pattern: BASE-QUOTE, e.g., BTC-USDT
    s = (instId or "")
    if "-" in s:
        a, b = s.split("-", 1)
...
def okx_balance_guard(*, symbol: str, side: str, qty: float) -> None:
    """Raise RuntimeError with standardized reason prefix when insufficient."""
    enable = _hotpatch_bool(os.getenv("OKX_BAL_GUARD_ENABLE", "1"), True)
    if not enable:
        return

    side_l = (side or "").lower()
    base_ccy, quote_ccy = _inst_base_quote(symbol)

    # SELL: need base asset qty
    if side_l == "sell":
        have = okx_avail_ccy(base_ccy)
        if have + 1e-12 < float(qty):
            raise RuntimeError(f"INSUFFICIENT_BAL: need {qty} {base_ccy}, have {have} {base_ccy}")
        return

    # BUY: need quote currency approx = qty * last_price (tgtCcy=base_ccy)
    px = okx_last_price(symbol, ttl_sec=int(os.getenv("OKX_PX_TTL", "10") or "10"))
    if px is None:
        # can't estimate safely -> block (safer) or allow (less safe)
        block = _hotpatch_bool(os.getenv("OKX_BLOCK_ON_NO_PRICE", "1"), True)
        if block:
            raise RuntimeError("PRICE_UNAVAILABLE: cannot estimate required quote amount")
        return

    slip = _hotpatch_float(os.getenv("OKX_BAL_GUARD_SLIPPAGE", "0.01"), 0.01)  # 1% default
    need_quote = float(qty) * float(px) * (1.0 + slip)
    have_quote = okx_avail_ccy(quote_ccy)
    if have_quote + 1e-8 < need_quote:
        raise RuntimeError(f"INSUFFICIENT_BAL: need~{need_quote:.6f} {quote_ccy} (qty={qty} px={px}), have {have_quote} {quote_ccy}")

# wrap okx_place_order (balance guard before submit)
if "okx_place_order" in globals() and "_okx_place_order_bal_guard_wrapped" not in globals():
    _okx_place_order_bal_guard_wrapped = True
    _okx_place_order_before_bal_guard = okx_place_order

    def okx_place_order(*, symbol: str, side: str, qty: float, order_type: str = "market", payload: dict | None = None) -> dict:
        # preflight already enforced by previous patch; balance guard now
        okx_balance_guard(symbol=symbol, side=side, qty=qty)
        return _okx_place_order_before_bal_guard(symbol=symbol, side=side, qty=qty, order_type=order_type, payload=payload)

# diag endpoint: show balances (masked) & price
try:
    if "app" in globals():
        @app.get("/api/diag/okx-balance-legacy")
        def diag_okx_balance():
            usdt = okx_avail_ccy("USDT")
            return {"ok": True, "avail": {"USDT": usdt}}
except Exception:
    pass


# ============================================================
# [HOTPATCH_W3D2_BAL_GUARD_FIX] ensure diag route + balance guard
# - Adds GET /api/diag/okx-balance
# - Guards okx_place_order to block submit when balance insufficient
# ============================================================

if "_okx_bal_guard_fix_loaded" not in globals():
    _okx_bal_guard_fix_loaded = True

    if "_okx_bal_cache2" not in globals():
        _okx_bal_cache2 = {"ts": 0.0, "data": None, "msg": ""}

    if "_okx_px_cache2" not in globals():
        _okx_px_cache2 = {}  # instId -> {"ts": float, "px": float}

    def _ps_bool(v: str, default: bool) -> bool:
        s = (v or "").strip().lower()
        if s in ("1","true","yes","y","on"): return True
        if s in ("0","false","no","n","off"): return False
        return default

    def _ps_float(v: str, default: float) -> float:
        try: return float((v or "").strip())
        except Exception: return default

    def okx_get_balances2(*, ttl_sec: int = 10) -> dict:
        import time
        now = time.time()
        try:
            if _okx_bal_cache2["data"] is not None and (now - float(_okx_bal_cache2["ts"])) < ttl_sec:
                return {"ok": True, "data": _okx_bal_cache2["data"], "msg": "cached"}
            ...
            _okx_bal_cache2.update({"ts": now, "data": j, "msg": "ok"})
            return {"ok": True, "data": j, "msg": "ok"}
        except Exception as e:
            _okx_bal_cache2.update({"ts": now, "data": None, "msg": str(e)})
            return {"ok": False, "data": None, "msg": str(e)}

    def okx_avail_ccy2(ccy: str) -> float:
        res = okx_get_balances2(ttl_sec=int(os.getenv("OKX_BAL_TTL", "10") or "10"))
        if not res.get("ok"):
            return 0.0
        j = res.get("data") or {}
        try:
    ...
    # === [HOTPATCH_W3D3_OKX_BAL_SPLIT] trading vs funding balance ===
    # OKX는 계정 구조(Trading vs Funding)에 따라 /account/balance(트레이딩)와 /asset/balances(펀딩)가 분리되어 보일 수 있습니다.
    # 사용자가 "USDT가 있는데 bot은 0으로 본다" 케이스를 명확히 하기 위해 두 소스를 함께 조회/표시합니다.

    if "_okx_asset_bal_cache2" not in globals():
        _okx_asset_bal_cache2 = {"ts": 0.0, "data": None, "msg": ""}

    def okx_get_asset_balances2(*, ccy: str | None = None, ttl_sec: int = 10) -> dict:
        """Funding(Asset) balances via /api/v5/asset/balances (TTL cache)."""
        import time
        now = time.time()
        try:
            if _okx_asset_bal_cache2["data"] is not None and (now - float(_okx_asset_bal_cache2["ts"])) < ttl_sec:
                return {"ok": True, "data": _okx_asset_bal_cache2["data"], "msg": "cached"}

            base, key, sec, pas, sim, timeout = _okx_env()
            q = ""
            if ccy:
                q = f"?ccy={str(ccy).upper()}"
            path = "/api/v5/asset/balances" + q
            url = base + path
            ts = _okx_ts()
            prehash = f"{ts}GET{path}"
            sign = _okx_sign(sec, prehash)
            ...
            _okx_asset_bal_cache2.update({"ts": now, "data": j, "msg": "ok"})
            return {"ok": True, "data": j, "msg": "ok"}
        except Exception as e:
            _okx_asset_bal_cache2.update({"ts": now, "data": None, "msg": str(e)})
            return {"ok": False, "data": None, "msg": str(e)}

    def _okx_asset_avail(j: dict, ccy: str) -> float:
        """Parse available balance from /asset/balances payload."""
        try:
            data = (j.get("data") or [])
            if not data:
                return 0.0
            ...
            for d in data:
                if (d.get("ccy") or "").upper() == ccy_u:
                    # OKX asset/balances uses 'availBal' and/or 'bal'
                    v = d.get("availBal")
    ...
    def okx_avail_ccy_split2(ccy: str) -> dict:
        """Return trading/funding/total available."""
        ccy_u = (ccy or "").upper()
        trading = 0.0
        funding = 0.0

        r1 = okx_get_balances2(ttl_sec=int(os.getenv("OKX_BAL_TTL", "10") or "10"))
        if r1.get("ok"):
            j1 = r1.get("data") or {}
            try:
                data = (j1.get("data") or [])
                if data:
                    ...
                    for d in details:
                        if (d.get("ccy") or "").upper() == ccy_u:
                            trading = float(d.get("availBal") or 0.0)
                            break
            except Exception:
                pass

        r2 = okx_get_asset_balances2(ccy=ccy_u, ttl_sec=int(os.getenv("OKX_BAL_TTL", "10") or "10"))
        if r2.get("ok"):
            j2 = r2.get("data") or {}
            funding = _okx_asset_avail(j2, ccy_u)

        return {"ccy": ccy_u, "trading": trading, "funding": funding, "total": trading + funding,
                "src": {"trading_ok": bool(r1.get("ok")), "funding_ok": bool(r2.get("ok"))},
                "src_msg": {"trading": str(r1.get("msg") or ""), "funding": str(r2.get("msg") or "")}}

    # Override: keep existing callers working (returns TRADING avail, because 주문은 트레이딩 계정 기준)
    def okx_avail_ccy2(ccy: str) -> float:
        return float(okx_avail_ccy_split2(ccy).get("trading") or 0.0)

    # Diag: show split balances (USDT + optional ccy)
    try:
        if "app" in globals() and "_okx_diag_balance_split2" not in globals():
            _okx_diag_balance_split2 = True

            @app.get("/api/diag/okx-balance")
            @app.get("/api/diag/okx-balance-split")
            def diag_okx_balance_split(ccy: str = "USDT"):
                out = okx_avail_ccy_split2(ccy)
                out_usdt = okx_avail_ccy_split2("USDT")
                return {"ok": True, "ccy": out, "usdt": out_usdt}
    except Exception:
        pass


    def okx_last_price2(instId: str, *, ttl_sec: int = 10) -> float | None:
        import time
        now = time.time()
        k = (instId or "").strip()
        if not k: return None
        try:
            hit = _okx_px_cache2.get(k)
            if hit and (now - float(hit.get("ts", 0.0))) < ttl_sec:
                return float(hit.get("px"))
            ...
            j = resp.json()
            if str(j.get("code")) != "0": return None
            data = j.get("data") or []
            if not data: return None
            px = float(data[0].get("last") or 0.0)
            if px <= 0: return None
            _okx_px_cache2[k] = {"ts": now, "px": px}
            return px
        except Exception:
            return None

    def _inst_base_quote2(instId: str) -> tuple[str, str]:
        s = (instId or "")
        if "-" in s:
            a, b = s.split("-", 1)
            return (a.upper(), b.upper())
    ...
    def okx_balance_guard2(*, symbol: str, side: str, qty: float) -> None:
        if not _ps_bool(os.getenv("OKX_BAL_GUARD_ENABLE", "1"), True):
            return

        side_l = (side or "").lower()
        base_ccy, quote_ccy = _inst_base_quote2(symbol)

        if side_l == "sell":
            sp = okx_avail_ccy_split2(base_ccy)
            if (not (sp.get("src") or {}).get("trading_ok")) and (not (sp.get("src") or {}).get("funding_ok")):
                m = sp.get("src_msg") or {}
                raise RuntimeError(f"BALANCE_FETCH_FAILED: cannot read OKX balances for {base_ccy}. trading_err={m.get('trading','')} funding_err={m.get('funding','')}")
            have_t = float(sp.get("trading") or 0.0)
            have_f = float(sp.get("funding") or 0.0)
            if have_t + 1e-12 < float(qty):
                # 사용자 잔고(펀딩)에 있는데 트레이딩이 0인 케이스를 명확히 안내
                if have_f + 1e-12 >= float(qty):
                    raise RuntimeError(f"INSUFFICIENT_TRADING_BAL: need {qty} {base_ccy}, trading {have_t} {base_ccy}, funding {have_f} {base_ccy} (transfer funding->trading)")
                raise RuntimeError(f"INSUFFICIENT_BAL: need {qty} {base_ccy}, have {have_t} {base_ccy}")
            return

        px = okx_last_price2(symbol, ttl_sec=int(os.getenv("OKX_PX_TTL", "10") or "10"))
        if px is None:
            if _ps_bool(os.getenv("OKX_BLOCK_ON_NO_PRICE", "1"), True):
                raise RuntimeError("PRICE_UNAVAILABLE: cannot estimate required quote amount")
            return

        slip = _ps_float(os.getenv("OKX_BAL_GUARD_SLIPPAGE", "0.01"), 0.01)
        need_quote = float(qty) * float(px) * (1.0 + slip)
        have_sp = okx_avail_ccy_split2(quote_ccy)
        if (not (have_sp.get("src") or {}).get("trading_ok")) and (not (have_sp.get("src") or {}).get("funding_ok")):
            m = have_sp.get("src_msg") or {}
            raise RuntimeError(f"BALANCE_FETCH_FAILED: cannot read OKX balances for {quote_ccy}. trading_err={m.get('trading','')} funding_err={m.get('funding','')}")
        have_quote_t = float(have_sp.get("trading") or 0.0)
        have_quote_f = float(have_sp.get("funding") or 0.0)
        if have_quote_t + 1e-8 < need_quote:
            if have_quote_f + 1e-8 >= need_quote:
                raise RuntimeError(f"INSUFFICIENT_TRADING_BAL: need~{need_quote:.6f} {quote_ccy} (qty={qty} px={px}), trading {have_quote_t} {quote_ccy}, funding {have_quote_f} {quote_ccy} (transfer funding->trading)")
            raise RuntimeError(f"INSUFFICIENT_BAL: need~{need_quote:.6f} {quote_ccy} (qty={qty} px={px}), have {have_quote_t} {quote_ccy}")

    # Wrap okx_place_order so any submit path is guarded
    if "okx_place_order" in globals() and "_okx_place_order_bal_guard2_wrapped" not in globals():
        _okx_place_order_bal_guard2_wrapped = True
        _okx_place_order_before_guard2 = okx_place_order

        def okx_place_order(*, symbol: str | None = None, side: str | None = None, qty: float | None = None, order_type: str = "market", payload: dict | None = None, **kwargs) -> dict:
            # compat: accept legacy kwargs (instId/ordType/sz/tdMode) without breaking
            ...
            if qty is None:
                if "qty" in kwargs:
                    ...
                    qty = float(kwargs.get("sz"))
            if (not order_type or str(order_type).strip() == "") and "ordType" in kwargs:
            ...
            qty_f = float(qty)
            okx_balance_guard2(symbol=symbol, side=side, qty=qty_f)
            return _okx_place_order_before_guard2(symbol=symbol, side=side, qty=qty_f, order_type=(order_type or "market"), payload=payload)

    # Ensure diag route exists
    try:
        if "app" in globals():
            @app.get("/api/diag/okx-balance-legacy")
            def diag_okx_balance():
                return {"ok": True, "avail": {"USDT": okx_avail_ccy2("USDT")}}
    except Exception:
        pass
...
def _maybe_send_to_broker(db, order_id: int):
    ...
    try:
        ...
        if isinstance(data, list) and data:
            ...
            okx_clord_id = data[0].get("clOrdId") or okx_clord_id
    except Exception:
        okx_order_id = None
...
# === AUTOFIX_OKX_PLACE_ORDER_COMPAT_FINAL ===
# 목적: 과거/외부 호출 경로에서 okx_place_order(instId=..., sz=..., ordType=...) 같은 legacy kwargs가
#       들어와도 TypeError 없이 동작하도록 최종 호환 래퍼를 강제합니다.
# 원칙: Hub 내부 표준 시그니처는 (symbol, side, qty, order_type, payload)이며,
#       instId/sz/ordType/action 등은 여기서 매핑 후 기존 okx_place_order(가드/프리플라이트 포함)로 위임합니다.
if "okx_place_order" in globals() and "_okx_place_order_compat_final_wrapped" not in globals():
    _okx_place_order_compat_final_wrapped = True
    _okx_place_order_compat_final_original = okx_place_order
    ...
    def okx_place_order(*,
        ...
        # payload 보정: limit price 등이 들어오면 payload로 흡수
        if payload is None and isinstance(kwargs.get("payload"), dict):
            payload = kwargs.get("payload")

        if payload is None:
            px = kwargs.get("px") or kwargs.get("price")
            if px is not None:
                payload = {"px": px}
        ...
        return _okx_place_order_compat_final_original(
            symbol=str(symbol),
            side=str(side),
            qty=float(qty),
            order_type=(order_type or "market"),
...
# ============================================================
# [W5_CONNECTOR_BRIDGE_OKX_V1]
# - Week5 Day1: connector standardization scaffold.
# - Keep main.py behavior stable by overriding okx_place_order/okx_get_order to use connectors/okx.py (urllib).
# ============================================================
try:
    from .connectors.okx import okx_place_order as _w5_okx_place_order
    from .connectors.okx import okx_get_order as _w5_okx_get_order
except Exception:
    _w5_okx_place_order = None
    _w5_okx_get_order = None

def okx_place_order(*, symbol: str, side: str, qty: float, order_type: str = "market", px: float | None = None, payload: dict | None = None) -> dict:
    if _w5_okx_place_order is None:
        raise RuntimeError("okx_connector_missing")
    return _w5_okx_place_order(symbol=symbol, side=side, qty=qty, order_type=order_type, px=px, payload=payload)

def okx_get_order(*, symbol: str, okx_order_id: str | None = None, okx_clord_id: str | None = None) -> dict:
    if _w5_okx_get_order is None:
        raise RuntimeError("okx_connector_missing")
    return _w5_okx_get_order(symbol=symbol, okx_order_id=okx_order_id, okx_clord_id=okx_clord_id)
