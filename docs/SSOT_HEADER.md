...
**목표**: 국내/해외/ETF 시장 선택 후 기술적·기본적 조건검색으로 종목 필터링
...
├─────────────────────────────────────────────────────────────┤
│  기술적 필터                                                 │
│  ├ 이동평균선: [▼ 5일선 위] [▼ 20일선 위] [▼ 골든크로스]   │
│  ├ RSI(14): [▼ 30 이하] [▼ 70 이상] [▼ 전체]               │
│  ├ 거래량: [▼ 평균 대비 200%+] [▼ 급증]                     │
│  └ 52주: [▼ 신고가 근접] [▼ 신저가 근접]                    │
│                                                              │
│  기본적 필터                                                 │
...
│  결과: 127개 종목                        [검색] [초기화]    │
├─────────────────────────────────────────────────────────────┤
│  종목명 ▼ | 현재가 | 등락률 | 거래량 | 시총 | PER | RSI     │
│  ─────────────────────────────────────────────────────────  │
│  삼성전자  | 82,000 | +1.2% | 12.3M | 489조 | 12.3 | 55     │
│  SK하이닉스| 178,000| +2.1% | 3.2M  | 129조 | 8.7  | 62     │
│  ...                                                         │
└─────────────────────────────────────────────────────────────┘
```
...
| 시장 | 소스 | 비고 |
|------|------|------|
| 국내 | 네이버 금융 API + KRX | 기존 시장분석 API 재활용 |
| 해외 | Finviz screener + Yahoo Finance | Finviz 무료 API 또는 스크래핑 |
| ETF | 네이버 ETF API | Phase 6에서 구축한 API 재활용 |

### 필터 항목

**기술적 지표**:
- 이동평균선: 5/10/20/50/200일선 위/아래, 골든크로스/데드크로스
- RSI(14): 과매도(30↓), 과매수(70↑), 범위 지정
- 거래량: 평균 대비 %, 급증 감지
- 52주 고저: 신고가/신저가 근접 %
