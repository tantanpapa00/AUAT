...
- Day 31: ✅ 완료 (AI 리포트 차트↔텍스트 지지/저항 통일 + ADX 패널 수정 + 제목 중복 제거)

## Day 32 진행사항 (2026-02-28)

### AI 종목추천 타임아웃 수정 ✅ DONE

**문제**: 앱에서 "추천 데이터를 불러올 수 없습니다" 에러 (curl은 정상)

**원인**: `enrich_financial_data`가 500개 종목에 대해 각각 네이버 API 호출 → 타임아웃

**수정**:
| 파일 | 변경 |
|------|------|
| `app/screener/kr_screener.py` | `_financial_cache` 시스템 구축 (서버 시작 시 PER/PBR/ROE/배당수익률 캐시) |
| `app/screener/kr_screener.py` | `apply_cached_financial()` 함수 추가 (네트워크 호출 없이 즉시 반환) |
| `app/screener/kr_screener.py` | `warmup_roe_cache()` 확장: 모바일 API + 데스크톱 ROE 크롤링 병합 |
| `app/main.py` | `enrich_financial_data` 대신 `apply_cached_financial` 사용 |

**결과**: 498개 종목 재무데이터 캐시, ROE 496개 수집, API 즉시 응답

---

### 해외(US) 저평가 가치주 추가 ✅ DONE

**요청**: US 종목추천에도 저평가 가치주 필터 적용 (PER≤15, ROE≥15%, PBR≤1.5)

**수정**:
| 파일 | 변경 |
|------|------|
| `app/screener/us_screener.py` | `fetch_sp500_valuation()` 추가: Finviz v=121 뷰에서 PER/PBR 수집 |
| `app/screener/us_screener.py` | `fetch_all_us_stocks()`에 valuation 데이터 병합 |
| `app/main.py` | US value 전략 조건 변경: `PER≤15, ROE≥15%, PBR≤1.5` |

**Finviz 뷰 정리**:
| 뷰 | 용도 | 데이터 |
|----|------|--------|
| v=111 | Overview | 섹터, 시총, 종목명, 가격 |
| v=121 | Valuation | PER, PBR, PEG, P/S |
| v=161 | Financial | ROE, ROA, Margins, Debt |

**결과**: US 저평가 가치주 1개 (Comcast: PER 5.7, ROE 22%, PBR 1.15)

---

### 다크/라이트 모드 토글 위치 이동 ✅ DONE

**요청**: 테마 토글을 "계정관리"에서 "앱정보"로 이동

**수정**:
| 파일 | 변경 |
|------|------|
| `pc-app/ui/index.html` | 계정관리에서 테마 설정 섹션 제거 |
| `pc-app/ui/index.html` | 앱정보 페이지에 "화면 설정" 카드 추가 |

**결과**: 설정 → 앱정보에서 다크/라이트 모드 전환 가능

---

### 업데이트 시스템 계획 수립

**핵심 원칙**: "사용자는 업데이트를 느끼지 못해야 한다"
...
사용자가 앱 실행
    ↓
백그라운드에서 업데이트 서버에 최신 버전 확인
    ↓
새 버전 있으면 → 알림 배너 표시 (강제 아님)
    "새로운 버전 1.2.0이 있습니다. [지금 업데이트] [나중에]"
    ↓
사용자가 [지금 업데이트] 클릭
    ↓
매매 진행 중인지 확인
    ├── 진행 중 → "매매 완료 후 업데이트됩니다" (대기)
    └── 아님 → 다운로드 시작 (프로그레스 바)
    ↓
다운로드 완료 → "앱을 재시작하면 적용됩니다. [지금 재시작] [나중에]"
```

**핵심 규칙**:
| 규칙 | 설명 |
|------|------|
| 강제 업데이트 금지 | 사용자가 선택할 때만 업데이트 |
| 매매 중 보호 | 활성 주문/포지션 있으면 업데이트 대기 |
| 백그라운드 다운로드 | 앱 사용 중에도 다운로드 가능 |
| 롤백 가능 | 업데이트 실패 시 이전 버전으로 자동 복구 |

**업데이트 서버**: 자체 VPS (비공개 배포 필요)
...
git pull → docker compose up -d --build
    ↓
빌드 중 API 중단 (수 초 ~ 수십 초)
    ↓
사용자: "추천 데이터를 불러올 수 없습니다" 😱
```
...
:latest   → 최신 빌드 (개발용)
:1.2.0    → 특정 버전 (운영용)
:stable   → 현재 운영 중인 안정 버전
:rollback → 직전 안정 버전 (롤백용)
```
...
| Phase | 내용 | 기간 |
|-------|------|------|
| Phase 1 | 헬스체크 API + Docker 태그 + 배포 스크립트 | 1~2일 |
| Phase 2 | Nginx 리버스 프록시 + Blue-Green 배포 + 스테이징 | 1주 |
| Phase 3 | Tauri 자동 업데이트 + 업데이트 서버 + 매매 중 보호 | 2주 |
| Phase 4 | 롤백 자동화 + 모니터링 + 전체 테스트 | 출시 전 |
...
| 커밋 | 메시지 |
|------|--------|
| ce5f51f | fix: AI 종목추천 타임아웃 수정 - 캐시 기반 재무데이터 적용 |
| c78f698 | fix: 재무 캐시에 ROE 크롤링 추가 (데스크톱 페이지) |
| a64c98a | feat: 해외(US) 저평가 가치주 필터 추가 - PER≤15, ROE≥15%, PBR≤1.5 |
| dd79047 | fix: Finviz Valuation 뷰 번호 수정 - v=141 → v=121 |
| 7cd4e7c | fix: v=121 컬럼 인덱스 수정 - P/E=cols[3], P/B=cols[7] |
| 47618ef | refactor: 다크/라이트 모드 토글을 계정관리에서 앱정보로 이동 |

---

## Day 31 진행사항 (2026-02-27)

### AI 리포트 지지/저항선 불일치 수정 ✅ DONE

**문제**: 차트 범례와 AI 텍스트의 지지/저항 값이 다름
- 차트: $25/$28 지지, $32/$34 저항 ✅ (정확)
- AI 텍스트: $30.09/$30.82 지지 ❌ (현재가 $30보다 높음)

**원인**: 두 곳에서 별도로 계산
- 차트: 스윙 포인트 + 볼륨 기반 정교한 알고리즘
- AI 프롬프트: 피보나치 기반 단순 계산 (이전 세션에서 수정했으나 여전히 불일치)

**수정 내역**:

| 파일 | 변경 |
|------|------|
| `app/main.py` | `_generate_ai_charts_sync`에서 `chart_data["support_levels"]`, `chart_data["resistance_levels"]` 반환 |
| `app/main.py` | 호출부에서 `tech["support_levels"] = chart_urls["support_levels"]` 덮어쓰기 |
| `app/main.py` | `_generate_claude_report`, `_generate_etf_report`에 `chart_data` 파라미터 추가 |

**결과**: 차트와 AI 텍스트에서 동일한 지지/저항 값 사용

---

### ADX 패널 "데이터 부족" 수정 ✅ DONE

**문제**: 5.2 추세차트 ADX 패널이 "ADX 데이터 부족 (최소 200일 필요)" 표시

**원인**: 조건이 `len(adx_series) >= display_len` (200)으로 너무 엄격

**수정**:
- 조건 완화: `len(adx_series) >= 30` (ADX 계산에 실제 필요한 최소 기간)
- 변수 참조 수정: `date_nums_adx = date_nums[-actual_display:]`

---

### "5. 기술적 분석" 제목 중복 제거 ✅ DONE

**문제**: PDF 4페이지에 "5. 기술적 분석" 제목이 2번 출력

**원인**: AI 텍스트의 `before_ta` 끝에 제목 포함 + 섹션 텍스트에도 포함 가능

**수정**:

| 위치 | 변경 |
|------|------|
| `before_ta` 처리 | 끝에서 "5. 기술적 분析" 정규식 제거 |
| `insert_section_with_chart` | 섹션 텍스트에서 "5. 기술적 분석" 제거하는 정규식 추가 |

---

### US ETF 자동완성 확장 ✅ DONE

**문제**: USHY, SCHG 등 해외 ETF가 자동완성에 표시되지 않음

**수정**:
- `US_ETFS` 목록 20개 → 80개 확장
- 추가된 ETF: USHY, SCHG, SCHB, SCHD, SCHY, SCHX, SCHF 등 주요 ETF

---

### 종목 인식 함수 근본 수정 ✅ DONE

**문제**: "SCHG 분석해줘" → "U (Unity)" 잘못 인식
- 원인: "Schwab U.S. Large-Cap Growth ETF"에서 "U"를 먼저 추출

**수정**:

| 항목 | 내용 |
|------|------|
| SKIP_WORDS | `{'A', 'I', 'U', 'AI', 'IT', 'CAP', ...}` — 단일 글자 및 일반 단어 필터 |
| ETF fullname 매칭 | 티커 추출 전에 ETF 전체 이름으로 먼저 매칭 시도 |
| 자동완성 선택 | 종목명 대신 티커 코드 사용 (`input.value = code + " 분석해줘"`) |
...
| 파일 | 변경 |
|------|------|
| `app/main.py` | 지지/저항 통일, ADX 조건 완화, 제목 중복 제거, ETF 목록 확장, 종목 인식 개선 |
| `pc-app/ui/src/main.js` | 자동완성 선택 시 티커 코드 사용 |

| 커밋 | 메시지 |
|------|--------|
| eca0806 | fix: 지지/저항선 역전 수정 + 기술적 분석 제목 중복 제거 |
| 5de6a3c | fix: 차트↔텍스트 지지/저항 값 통일 + 제목 중복 제거 |
...
### Phase 8-2 확장: FnGuide 6년 데이터 + EPS 지배주주 기준 ✅ DONE
...
**EPS 지배주주 기준 수정 (stockeasy 완전 일치)**:
- FnGuide Table 11 (연간 6년) 파싱으로 변경
- Table 10 (분기 포함) 건너뛰기 조건 추가
- 분기 데이터(/03, /06, /09) 필터링
- HMM EPS 검증:
  - 2023: 1,829원 → **1,632원** ✓
  - 2024: 5,055원 → **4,293원** ✓
- 6개 연도 표시: 2022~2027(E) ✓

**미완료 (다음 세션)**:
- GPM 라인: SVD_Finance에서 매출총이익 추가 파싱 필요
- EPS 전망추이 라인차트 (날짜별): Naver/FnGuide에 데이터 없음
...
| 2502c6c | fix: EPS 지배주주 기준 완전 수정 + 6년 데이터 확보 |
