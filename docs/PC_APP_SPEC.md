...
# 10) 백테스트 결과 UI (Day 15)

## 10-1) 결과 화면 구성 (TradingView 동일)
...
│                                                                     │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐       │
│  │ 총손익  │ │ 최대자본│ │ 총거래  │ │ 수익성  │ │ 수익지수│       │
│  │ +15.2%  │ │ -8.3%   │ │ 16회    │ │ 68.75%  │ │ 1.82    │       │
│  │ +1,520  │ │ -830    │ │         │ │ 11/16   │ │         │       │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘       │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
...
│  │  거래 내역                                                     │  │
│  │  ┌──────┬──────┬────────┬────────┬─────────┬─────────┐        │  │
│  │  │ #    │ Side │ Symbol │ Price  │ 수익금  │ 수익률  │        │  │
│  │  ├──────┼──────┼────────┼────────┼─────────┼─────────┤        │  │
│  │  │ 1    │ BUY  │ BTC    │ 42,500 │ +320    │ +3.2%   │        │  │
│  │  │ 2    │ SELL │ BTC    │ 43,860 │ -        │ -       │        │  │
│  │  │ 3    │ BUY  │ BTC    │ 41,200 │ -85     │ -0.85%  │        │  │
│  │  │ ...  │      │        │        │         │         │        │  │
│  │  └──────┴──────┴────────┴────────┴─────────┴─────────┘        │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
...
## 10-2) 상단 카드 5개 (Summary)

| 카드 | 필드 | 계산 |
|------|------|------|
| 총손익 | `total_pnl`, `total_pnl_pct` | 금액 + % |
| 최대자본감소 | `max_drawdown`, `max_drawdown_pct` | MDD 금액 + % |
| 총거래횟수 | `trade_count` | 라운드트립 수 |
| 수익성거래 | `win_rate`, `win_count/total` | % + n/n |
| 수익지수 | `profit_factor` | 총수익/총손실 |

## 10-3) 수익률 테이블 (전체/매수/매도 3열)
...
| 총손실 | gross_loss |
| 수익지수 | profit_factor |
| 수수료 | total_commission (기본 0.1%) |
| 기대수익 | expected_value (평균 수익) |

## 10-4) 자본 차트 (Equity Curve)
...
| Day 15 | 백테스트 벡터화 최적화 + 빌드 성공 | DONE |
